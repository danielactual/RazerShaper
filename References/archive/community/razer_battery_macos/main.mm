...
- (void)applicationDidFinishLaunching:(NSNotification*)notification {
    ...
    NSMenuItem* versionItem_ = [[NSMenuItem alloc] initWithTitle:@"Version: 1.3.5" action:nil keyEquivalent:@""];
...
- (void)handleUSBEvent {
    ...
    uint8_t cachedLevel = lastBatteryLevel_;

    dispatch_async(batteryQueue_, ^{
        BatteryMonitorApp* s = weakSelf;
        if (!s || !s->razerDevice_) return;

        // Disconnect stale USB handle and reconnect
        s->razerDevice_->disconnect();
        bool connected = s->razerDevice_->connect();

        if (!connected) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf setDisconnectedState:@"Disconnected"];
            });
            return;
        }

        // Query battery on background thread (never block main thread with USB I/O)
        uint8_t batteryPercent = 0;
        bool success = s->razerDevice_->queryBattery(batteryPercent);
        bool isCharging = false;
        s->razerDevice_->queryChargingStatus(isCharging);
        if (!isCharging && s->razerDevice_->isWiredDevicePresent()) {
            isCharging = true;
        }

        dispatch_async(dispatch_get_main_queue(), ^{
            BatteryMonitorApp* ms = weakSelf;
            if (!ms) return;
            if (success && batteryPercent > 0) {
                [ms updateBatteryDisplayWithLevel:batteryPercent charging:isCharging];
            } else if (cachedLevel > 0) {
                [ms updateBatteryDisplayWithLevel:cachedLevel charging:isCharging];
            } else if (isCharging) {
                [ms updateBatteryDisplayWithLevel:0 charging:true];
            } else {
                [ms setDisconnectedState:@"Disconnected"];
            }
        });
    });
}
...
- (NSTimeInterval)pollIntervalForBattery:(uint8_t)battery charging:(bool)charging {
    ...
    if (battery <= 20 && battery > 0) return 3.0;   // Lav batteri → sjekk ofte
    return 10.0;                          // Normal
}
...
- (void)connectToDevice {
    ...
    dispatch_async(batteryQueue_, ^{
        ...
        if (!s->razerDevice_->connect()) {
            bool needsPriv = s->razerDevice_->needsPrivileges();
            dispatch_async(dispatch_get_main_queue(), ^{
                BatteryMonitorApp* ms = weakSelf;
                if (!ms) return;
                if (needsPriv) {
                    [ms setDisconnectedState:@"⚠️ Trenger admin"];
                    ...
                    [ms setDisconnectedState:@"Ingen Razer mus funnet"];
                    NSLog(@"Failed to connect to Razer device");
                    [ms performSelector:@selector(connectToDevice) withObject:nil afterDelay:10.0];
                }
            });
            return;
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            BatteryMonitorApp* ms = weakSelf;
            if (!ms) return;
            [ms updateBatteryDisplay];
            if (!ms->pollTimer_) {
                [ms schedulePollWithInterval:5.0];
            }
        });
    });
}
    ...
    dispatch_async(batteryQueue_, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (success && batteryPercent > 0) {
                ...
                NSLog(@"WARNING: Using cached battery level: %d%%", cachedLevel);
                [ms updateBatteryDisplayWithLevel:cachedLevel charging:isCharging];
            } else if (isCharging) {
                NSLog(@"WARNING: Battery unknown, cable detected — showing charging state");
                [ms updateBatteryDisplayWithLevel:0 charging:true];
            } else {
                [ms setDisconnectedState:@"Battery query failed"];
            }
        });
    });
}
...
- (void)checkAccessibilityPermission:(NSTimer*)timer {
    (void)timer;
    BOOL hasPermission = [ScrollInterceptor hasAccessibilityPermission];
    NSLog(@"checkAccessibilityPermission called, hasPermission=%d", hasPermission);
...
- (void)scrollFeatureToggled:(id)sender {
    ...
    // Rebuild scroll settings menu to update enabled states and slider visibility
    scrollSettingsMenu_ = [self buildScrollSettingsMenu];
    for (NSMenuItem* item in statusItem_.menu.itemArray) {
        if ([item.title hasPrefix:@"Scroll Settings"]) {
            [item setSubmenu:scrollSettingsMenu_];
            break;
        }
    }
    [self updateStatusBarTooltip];
